...
8. If the Cloud Foundry org or space to which you are planning to deploy has special characters in the name, open the file xs-security.json under the shine-cf project root folder and edit line no. 2 `${org}-${space}-shine-admin`. Change the value to `"your-org-name-without-special-characters"-"your-space-name-without-special-characters"-shine-admin`, e.g. `p1726482trial_trial-dev_cf-shine-admin` althrough my org name is "p1726482trial_trial" and space name is "dev@cf"("-" and "_" are not counted as special characters), this is so that the XSUAA has a unique identitfier for your application (and it can't handle some special characters)
