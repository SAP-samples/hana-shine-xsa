...
describe(__filename, () => {
	...
	it("Valid Value Test", async(done) => {
		try {
	...
	it("Invalid Value Test", async(done) => {
		try {
			let sql = "SELECT * FROM \"POTextSearch\"(?,?)";
			let statement = await this.db.preparePromisified(sql);
			let results = await this.db.statementExecPromisified(statement, ["HT-1000", "COMPANYNAME"]);
			expect(results.length).toBeLessThan(1);
			done();
		} catch (err) {
			done.fail(err);
		}
