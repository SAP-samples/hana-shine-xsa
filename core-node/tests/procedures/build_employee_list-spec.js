...
describe(__filename, () => {
	...
	beforeEach(async(done) => {
		...
		this.sp = await this.test.getStoredProc(this.db, "build_employee_list");
		done();
	});
	...
	it("Value Test", async(done) => {
		try {
			...
			expect(results.results.length).not.toBeLessThan(2);
			expect(results.results[0].EMPLOYEEID).toBe("1000001");
			done();
