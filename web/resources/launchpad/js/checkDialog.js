...
sap.account.CheckDialog.prototype.open = function () {
	...
	oCheckDialog.roleCollectionBtn = new sap.ui.commons.Button({
		...
		press: function () {
			...
			var jobrole = {
				roleTemplateAppId: "jobscheduler",
				roleTemplateName: "jobscheduler_administration_template",
				name: "jobscheduler_administration_template"
			};
				...
				if (applications != undefined) {
					if (applications.length != undefined && applications.length > 0) {
						...
						for (var i = 0; i < applications.length; i++) {
							if ((applications[i].appid).indexOf("shine!") > -1 || (applications[i].appid).indexOf("shine") > -1) {
								applicationFound = true;
								var role = {
									roleTemplateAppId: applications[i].appid,
									roleTemplateName: "shine_admin",
									name: "shine_admin"
								};
